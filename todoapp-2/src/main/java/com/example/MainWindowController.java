...
public class MainWindowController implements Initializable{
    ...
    //testing
    @FXML private Label dayTaskCountLabel;
    @FXML private Label checkedTaskCountLabel;
    private Callback <Task, BooleanProperty> selectedPropertyCallback = task -> task.completedProperty();
    ...
    //more tests
    private void updateCheckedTaskCount() {
        int checkedCount = 0;  //declaring outside of if statement
        if (selectedPropertyCallback != null) {
            checkedCount = taskListView.getItems().stream()
                .map(item -> selectedPropertyCallback.call(item).get())
                .filter(Boolean::booleanValue)
                .mapToInt(selected -> selected ? 1 : 0)
                .sum();
        ...
        checkedTaskCountLabel.setText("Checked tasks: " + checkedCount); 
    }
    ...
    public void updateTaskListView(LocalDate date) {
        ...
        taskListView.setCellFactory(CheckboxListCell.forListView(task -> {
            BooleanProperty property = task.completedProperty();
            property.addListener((observable, oldValue, newValue) -> {
                // Update the checked task count when completion status changes
                updateCheckedTaskCount();
            });
            return property;
        }));

        //counter for task count
        int dayTaskCount = tasks.size();
        dayTaskCountLabel.setText("Tasks for this day: " + dayTaskCount);
    }
    ...
    //declaring increase progress method
    public void increaseProgress(){
        ...
        int completedTasks = (int) tasks.stream()
            ...
            .filter(Boolean::booleanValue)
            .count();
        double calculatedProgress = (double) completedTasks / totalTasks;
        if (progress.doubleValue() < 1){
            progress = new BigDecimal(String.format("%.2f", calculatedProgress ));
            myProgressBar.setProgress(progress.doubleValue());
            completionLabel.setText(Integer.toString((int)Math.round(calculatedProgress * 100)) + "%");
